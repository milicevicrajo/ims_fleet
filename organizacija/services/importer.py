...
# Naziv drugog nivoa jos nije potvrdjen (otvoreno pitanje iz popisa), pa se ne izmislja.
UNIT_NAME_UNKNOWN = ""
...
def _ensure_centers(company, known_units, needed, valid_from):
    """Prvi nivo. Kljuc mapiranja je sama oznaka jedinice."""
    ...
    for unit in sorted(needed):
        ...
        version_made = _ensure_version(
            ...
            segment=unit,
            full_code=unit,
            name=known_units[unit],
            is_active=True,
            is_profit=None,
            valid_from=valid_from,
            note="Naziv iz knjizenja (organizational_unit_name).",
        )
...
def _import_jobs(company, classified, centers, valid_from, run):
    ...
    for row, result in classified:
        ...
        if result.family != klas.FAMILY_BUSINESS:
        ...
        unit_code = center_code + unit_digit
