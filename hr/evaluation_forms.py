...
from hr.services.evaluations import editable_employees, MONTHS
...
class EvaluationHeaderForm(forms.Form):
    ...
    def __init__(self,*args,user,**kwargs):
        ...
        for field in ['employee','supervisor','director','general_director']:
            self.fields[field].label_from_instance = lambda employee: f'{employee.employee_code} — {employee}'
        style_form(self)
...
class EvaluationValuesForm(forms.Form):
    ...
    comment = forms.CharField(required=False,max_length=4000,label='Општи коментар',widget=forms.Textarea(attrs={'rows':3}))

    def __init__(self,*args,snapshot,token=None,**kwargs):
        super().__init__(*args,**kwargs)
        self.fields['snapshot_token'].initial = token
        self.fields['comment'].initial = snapshot.get('comment','')
        for item in snapshot['items']:
            ...
            self.fields[name] = forms.ChoiceField(choices=choices,label=item['name'],initial=str(item['points']))
            self.fields[name].widget.attrs.update({'class':'form-control form-select evaluation-points','data-code':item['code']})
            self.fields[f'comment_{item["code"]}'] = forms.CharField(required=False,max_length=2000,label='Коментар оцењивача',initial=item.get('comment',''),widget=forms.Textarea(attrs={'class':'form-control','rows':2}))
...
class EvaluationApprovalForm(forms.Form):
    stage = forms.ChoiceField(label='Ниво сагласности',choices=[('supervisor','Непосредни руководилац'),('director','Директор центра'),('general_director','Генерални директор')])
    document_date = forms.DateField(label='Датум на обрасцу',widget=forms.DateInput(attrs={'type':'date'},format='%Y-%m-%d'))
    coefficient = CoefficientField(required=False,max_digits=10,decimal_places=5,label='Коначни коефицијент (опциона корекција)',widget=forms.TextInput(attrs={'inputmode':'decimal'}))
    comment = forms.CharField(required=False,max_length=4000,label='Образложење / коментар',widget=forms.Textarea(attrs={'rows':2}))
    confirm = forms.BooleanField(label='Сагласан/сагласна сам са овом верзијом и наведеним коефицијентом.')
