...
| 11 | Poslovni i naučni blok su centri **`2` i `3`** (šifarnik, Flota, registar), a knjiženja ih vode kao jedinice `20` i `30` — `OZNAKA_CENTRA_IZ_KNJIZENJA`. Naučna šifra je `3` + šifra radnika iz Kadrova + broj projekta i deo je bloka 3: jedinica je naučni projekat (`3-702400`), šifra je učešće radnika, a radnik se vezuje za Kadrove po ličnom broju. Nazivi su po Pravilniku o organizaciji (`organizacija/services/pravilnik.py`), latinicom. `111111` je tehnička šifra zatvaranja i početnog stanja — bez centra; `110002`/`430001` su centri 11/43 iako im u `posao.blok` centar nije upisan Ne „ispravljaj“ `2` u `20`: iz oznake se prave brojevi putnih naloga (`2/2026-…`) i po njoj se ograničava pristup |
